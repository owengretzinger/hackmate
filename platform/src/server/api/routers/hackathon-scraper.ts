...
export const hackathonScraperRouter = createTRPCRouter({
  scrapeHackathon: publicProcedure
    ...
    .mutation(async ({ input }) => {
      // Only allow scraping in development mode
      if (env.NODE_ENV === "production") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Hackathon scraping is only available in development mode",
        });
      }
        ...
        try {
          ...
          if (!response?.ok()) {
            ...
            await page.screenshot({
              path: path.join(debugDir, "error-page.png"),
              fullPage: true,
            });
            throw new Error(
              `Failed to load page: ${response?.status()} ${response?.statusText()}`,
          ...
          await page.screenshot({
            path: path.join(debugDir, "initial-load.png"),
            fullPage: true,
          });
          ...
          const selectors = await page.evaluate(() => {
            ...
            elements.forEach((el) => {
              getAllSelectors(el).forEach((selector) =>
          ...
          await page.waitForSelector(".software-entry, .gallery-item", {
            timeout: 60000,
          });
          ...
          await page.screenshot({
            path: path.join(debugDir, "projects-loaded.png"),
            fullPage: true,
          });
          ...
          const projects = await page.evaluate(() => {
            ...
            return Array.from(entries).map((entry) => {
              ...
              const tagline =
                entry
                  .querySelector("p, .software-entry-description")
                  ?.textContent?.trim() ?? "";
              const linkElement = entry.querySelector(
                'a.link-to-software, a[href*="/software/"]',
              );
              const devpostUrl =
              ...
              const isWinner =
                entry.querySelector(
                  '.winner-badge, .winner, .winner-banner, [class*="winner"]',
                ) !== null;

              // Get thumbnail URL
              const thumbnailImg = entry.querySelector(
                ".software_thumbnail_image",
              );
              const thumbnailUrl =
                thumbnailImg instanceof HTMLImageElement
                  ? thumbnailImg.src
                  : null;
          ...
          const uniqueProjects = projects.reduce(
            (acc, project) => {
              if (
                project.devpostUrl &&
                !acc.some((p) => p.devpostUrl === project.devpostUrl)
              ) {
                acc.push(project);
              }
              return acc;
          ...
          for (const project of winningProjects) {
            ...
            try {
              await projectPage.goto(project.devpostUrl, {
              ...
              await writeFile(
                path.join(
                  debugDir,
                  `project-${project.title.replace(/[^a-z0-9]/gi, "_")}-page.html`,
                ),
                pageContent,
              );
              ...
              const details = await projectPage.evaluate(() => {
                ...
                const articleContent = document.querySelector(
                  "#app-details-left > div:not(#gallery):not(#built-with):not(.app-links)",
                );
                let fullDescription = "";
                if (articleContent) {
                  ...
                  console.log(
                    "Extracted formatted description:",
                ...
                const teamMembers = Array.from(
                  document.querySelectorAll(".software-team-member"),
                ).map((member) => {
                  // Get name and profile URL - look for the link within the large-8 columns div
                  const profileLink = member.querySelector(
                    ".small-10.large-8.columns .user-profile-link",
                  );
                  const name =
                    profileLink?.textContent?.trim() ?? "Unknown Member";
                  const profileUrl =
                  ...
                  const avatarUrl =
                    avatar instanceof HTMLImageElement ? avatar.src : undefined;
                ...
                const techNodes = document.querySelectorAll(
                  ".cp-tag, .built-with-list li",
                );
                const technologies = Array.from(techNodes)
                  .map((node) => {
                ...
                const galleryNodes = document.querySelectorAll(
                  "#gallery .slick-slide:not(.slick-cloned) a[data-lightbox]",
                );
                const galleryImages = Array.from(galleryNodes)
                  .map((node) => ({
                ...
                const demoVideo =
                  videoFrame instanceof HTMLIFrameElement
                    ? {
                        url: videoFrame.src ?? "",
                        type: videoFrame.src?.includes("youtube")
                          ? ("youtube" as const)
                          : videoFrame.src?.includes("vimeo")
                            ? ("vimeo" as const)
                            : ("other" as const),
                        videoId: videoFrame.src
                          ? (new URL(videoFrame.src).searchParams.get("v") ??
                            undefined)
                          : undefined,
                      }
                    : null;
                ...
                const likeCount = parseInt(
                  document.querySelector(".software-likes .side-count")
                    ?.textContent ?? "0",
                  10,
                );
                const commentCount = parseInt(
                  document.querySelector(".comment-button .side-count")
                    ?.textContent ?? "0",
                  10,
                );
                ...
                const awards = Array.from(
                  document.querySelectorAll(
                    "#submissions .software-list-content .winner",
                  ),
                ).map((node) => {
                  const label = node.textContent?.trim() ?? "";
                  const prizeText = node.nextSibling?.textContent?.trim();
                  const section = node.closest("section, .category-section");
                  const categoryHeading = section?.querySelector(
                    "h1, h2, h3, .category-name",
                  );
                  const category =
                    categoryHeading?.textContent?.trim() ?? "Overall";
                ...
                const softwareUrls = document.querySelector(
                  '[data-role="software-urls"]',
                );
                ...
                for (const selector of githubSelectors) {
                  ...
                  if (link instanceof HTMLAnchorElement) {
                    githubLink = link;
                    console.log(
                      `Found GitHub link using selector: ${selector}`,
                    );
                    break;
                  }
                }
                ...
                const websiteLink = document.querySelector(
                  '[data-role="software-urls"] a[href]:not([href*="github.com"])',
                );
                console.log(
                  "Found website link:",
                ...
                return {
                  ...
                  websiteUrl:
                    websiteLink instanceof HTMLAnchorElement
                      ? websiteLink.href
                      : null,
                  debugInfo: {
              ...
              console.log(
                `📝 Description length: ${details.fullDescription?.length ?? 0} characters`,
              );
              console.log(
                `👥 Team members found: ${details.teamMembers.length}`,
              );
              console.log(
                `🏷️ Technologies found: ${details.technologies.length}`,
              );
              console.log(
                `🖼️ Gallery images found: ${details.galleryImages.length}`,
              );
              ...
              await writeFile(
                path.join(
                  debugDir,
                  `project-${project.title.replace(/[^a-z0-9]/gi, "_")}.json`,
                ),
                JSON.stringify(
                  {
                    title: project.title,
              ...
              try {
                ...
                console.log(
                  `✅ Successfully stored/updated project: ${project.title}`,
                );
                successCount++;
              } catch (error) {
                console.error(
                  `❌ Error storing project: ${project.title}`,
                  error,
                );
                errorCount++;
              }
              ...
              console.error(
                `❌ Error fetching details for ${project.title}:`,
                error,
              );
